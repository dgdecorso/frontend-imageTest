    ...
    if (token) {
      ...
      fetch('http://localhost:8080/user/profile', { headers })
        ...
        .then(userData => {
          ...
          fetch('http://localhost:8080/user', { headers })
            .then(res => {
            ...
            .catch(error => {
              ...
              if (!adminCheck) {
                alert('Sie haben keine Admin-Berechtigung für die Benutzerverwaltung.');
                navigate('/');
              } else {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Sind Sie sicher, dass Sie diesen Benutzer löschen möchten?')) {
      try {
        ...
        alert('Fehler beim Löschen des Benutzers. Überprüfen Sie Ihre Admin-Berechtigung.');
      }
    }
  };
